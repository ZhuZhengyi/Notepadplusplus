...
void ScintillaEditView::foldChanged(int line, int levelNow, int levelPrev)
{
	...
	{
		if (!(levelPrev & SC_FOLDLEVELHEADERFLAG)) // But previously couldn't
		{
			// Adding a fold point.
			execute(SCI_SETFOLDEXPANDED, line, 1);
	...
	*/
	...
	else
	{
		...
		for(int i = searchStart; i < maxLines; ++i)
		{
			...
			if (levelLine & SC_FOLDLEVELHEADERFLAG) // Fold section. Don't show lines if fold is closed
			{
